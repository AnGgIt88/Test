...
#Toolchain
git clone --depth=1 https://github.com/AnGgIt86/arm64-gcc $(pwd)/NFS-Toolchain

# Main Declaration
KERNEL_ROOTDIR=$(pwd)/$DEVICE_CODENAME
DEVICE_DEFCONFIG=$DEVICE_DEFCONFIG
GCC_ROOTDIR=$(pwd)/NFS-Toolchain
...
tg_post_msg "<b>Finix-Kernel-Compiller</b>%0ABuilder Name : <code>${KBUILD_BUILD_USER}</code>%0ABuilder Host : <code>${KBUILD_BUILD_HOST}</code>%0ADevice Defconfig: <code>${DEVICE_DEFCONFIG}</code>%0AGCC Version : <code>${KBUILD_COMPILER_STRING}</code>%0AGCC Rootdir : <code>${GCC_ROOTDIR}</code>%0AKernel Rootdir : <code>${KERNEL_ROOTDIR}</code>"

# Compile
compile(){
tg_post_msg "<b>Finix-Kernel-Compiller:</b><code>Compilation has started</code>"
cd ${KERNEL_ROOTDIR}
